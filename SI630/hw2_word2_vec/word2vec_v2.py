...
def trainer(curW1 = None, curW2=None):
    global uniqueWords, wordcodes, fullsequence, vocab_size, hidden_size, np_randcounter, randcounter, context_window
...
def load_model():
    handle = open("saved_W1_sz1_1.data","rb")
    W1 = np.load(handle)
    handle.close()
    handle = open("saved_W2_sz1_1.data","rb")
    W2 = np.load(handle)
    handle.close()
    return [W1,W2]
...
def save_model(W1,W2):
    handle = open("saved_W1_sz2_1.data","wb+")
    np.save(handle, W1, allow_pickle=False)
    handle.close()

    handle = open("saved_W2_sz2_1.data","wb+")
...
def eval_pred():
    ...
    for targ in targets:
        ...
        print ("\n")
    filename = "p8_output.txt" if len(context_window) is 2 else "p9_output.txt"
...
def eval_intrinsic(test_data):
    ...
    write_output(eval, ['id', 'similarity'], "intrinsic-test_sz2.csv")
